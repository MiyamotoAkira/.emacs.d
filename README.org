...
We force the upgrading of packages built into emacs
#+BEGIN_SRC emacs-lisp
...
(defun set-size-font (size)
  ;; Set :family (not a combined :font string) so the default face always has an
  ;; explicit family. Otherwise posframe child frames inherit :family nil and
  ;; spam "Invalid face attribute :family nil". :height is in 1/10 pt.
  (set-face-attribute 'default nil
                      :family "Iosevka Curly Extended"
                      :height (* size 10)))
...
#+END_SRC
*** Mode line
doom-modeline replaces the abandoned spaceline-all-the-icons, which spammed
"Invalid face attribute :family nil" on every redisplay. Icons come from
nerd-icons — run =M-x nerd-icons-install-fonts= once if glyphs look wrong.
#+BEGIN_SRC emacs-lisp
...
** Main Org Section
Org itself is installed and loaded by =init.el=, before =org-babel-load-file= runs -- otherwise
the built-in Org loads first and a later ELPA Org gets byte-compiled against it, which produces
the "Org version mismatch" warning on every startup (=fix-org.sh= repairs that if it happens).
So this block only configures Org, hence =:ensure nil=.
#+BEGIN_SRC emacs-lisp
  (use-package org
    :ensure nil
    :defer 2
...
#+BEGIN_SRC emacs-lisp
(defun jgg/ag-project-no-tests (string)
    "Like `ag-project', but ignore files whose name contains \"test\"."
    (interactive (list (ag/read-from-minibuffer "Search string")))
    (let ((ag-arguments (append ag-arguments '("--ignore" "*test*"))))
      (ag-project string)))
...
(use-package ag
  :bind (("C-c a a" . ag)
         ...
         ("C-c a r" . ag-regex)
         ("C-c a p" . ag-project)
         ("C-c a P" . jgg/ag-project-no-tests))
  :config
...
Both =treesit-auto= and our manual =auto-mode-alist= claim =.js=, so a file can
end up in either =js-ts-mode= or =typescript-ts-mode=. They read different
indent variables (=js-indent-level= defaults to 4, =typescript-ts-mode-indent-offset=
defaults to 2), so we pin both to 2 to get consistent indentation regardless of
which mode wins.
#+BEGIN_SRC emacs-lisp
...
(use-package lsp-ui
  ...
  :bind (:map lsp-ui-mode-map
              ...
              ("C-S-n" . jgg/lsp-ui-doc-scroll-down)
              ("C-S-p" . jgg/lsp-ui-doc-scroll-up))
  ...
  :config
  (defun jgg/lsp-ui-doc-scroll-down (n)
    (interactive "p")
    (when-let* ((frame (lsp-ui-doc--get-frame))
                (win (frame-selected-window frame)))
      (with-selected-window win
        (scroll-up n))))
  (defun jgg/lsp-ui-doc-scroll-up (n)
    (interactive "p")
    (when-let* ((frame (lsp-ui-doc--get-frame))
                (win (frame-selected-window frame)))
      (with-selected-window win
...
(use-package python
  :hook ((python-ts-mode . eglot-ensure)
         ;; pylsp answers textDocument/signatureHelp with [] (an empty array)
         ;; instead of null, which trips eglot's plist handling:
         ;; "Error running timer: (wrong-type-argument plistp [])".
         ;; Ignore that capability for pylsp so eglot never issues the request.
...
Disabled: eglot manages Python completion via company-capf. Running company-jedi
as a second backend on the idle timer caused "Error running timer:
